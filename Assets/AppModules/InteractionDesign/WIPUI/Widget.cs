...
public class Widget : MonoBehaviour
{
  ...
  void Start() {
    handle.OnHoldBegin += onHandlePickedUp;
    //handle.OnMoved += onHandleMoved;
    handle.OnHoldEnd += onHandlePlaced;
    //handle.OnHandleHoldEnd += onHandleThrown;
  ...
  private void initiateMovementToTarget() {
    var targetPose = targetPoseProvider.GetPose();

    movementToPose.MoveToTarget(targetPose);
  }

  private void onMovementUpdate() {
    movementToPose.targetPose = new Leap.Unity.Pose() {
      position = movementToPose.targetPose.position,
      rotation = targetPoseProvider.GetPose().rotation
    };
  }
  ...
  public void MoveTo(Transform t) {
    Leap.Unity.Pose followingPose = t.ToWorldPose();
